...
namespace jcc {
	...
	/** The Element class used to send data from the server to client.
	 * Each new portion of data passed to the client when he sends /ping request.
	 * \code
	...
	class Element {
		...
		std::string elmName;
	public:
		Element(const std::string& name, LocalServer* ls);

		void operator <<(LocalServer& e);
		
		template <class X>
		Element& operator <<(const X& value);	
	...
	public:
		...
		LocalServer(int preferred_port = -1, int ping_time_ms = 300);
		~LocalServer();
		...
		/// starts to listen the port. Usually, you don't need to call it manually. Any other network interaction function calls it. And if you will call it, nothing wrong will happen.
		/// If it was already called and listening thread created, then nothing happens.
		void listenAsync();
	...
	*/
	...
	inline Element& LocalServer::console() {
		return el("console");
	}

	inline Element& LocalServer::el(const std::string& domElementID) {
		std::scoped_lock lk(jsonMutex);
		for (auto el : elements) {
			if (*el == domElementID)return *el;
		}
		Element* el = new Element(domElementID, this);
		elements.push_back(el);
		return *el;
	}

	inline Element& LocalServer::eval() {
		return el("$$$evaluate");
	}
	...
	inline Element::Element(const std::string& name, LocalServer* ls) {
		elmName = name;
		origin = ls;
	}

	inline void Element::operator<<(LocalServer& e) {
		std::scoped_lock lk(origin->jsonMutex);
		json::JSON el;
		el[elmName] = ss.str();
		origin->accumulator.append(el);
		ss.str(std::string());
	}

	inline bool Element::operator==(const std::string& id) {
		return elmName == id;
	}

	template <class X>
	Element& Element::operator<<(const X& value) {
		ss << value;
		return *this;
	}

	inline LocalServer::LocalServer(int preferred_port, int ping_time_ms) {
		listenThread = nullptr;
		_allowConsoleOutput = false;
		_autoTerminate = true;
		_stop = false;
		_pinged = false;
		pingTime = ping_time_ms;
		...
		svr->Get("/ping",
			[=](const httplib::Request& req, httplib::Response& res) {
				...
				{
					std::scoped_lock lk(jsonMutex);
					dump = accumulator.dump();
					accumulator = json::Array();
				}
	...
	inline LocalServer::~LocalServer() {
		for (auto el : elements) {
			delete(el);
		}
		elements.clear();
	...
	inline void LocalServer::open(const Html& page) {
		...
		lib = _replace(lib, "pingTimeMs", std::to_string(pingTime));
		if (strstr(home, "<body>")) {
	...
	inline void LocalServer::listenAsync() {
		std::scoped_lock lk(thread_mutex);
		if (svr && !listenThread) {
	...
	inline bool LocalServer::wait(int ms_amount)	{
		...
		if (ms_amount < 0) {
			...
			else thread_mutex.unlock();
		}
		return false;
	}

	inline bool LocalServer::alive(int ms_max) {
		return msSincePing() < ms_max;
	}
